...
_display_deployment_correctness_status() {
  ...
  kubectl -n "$_cluster_namespace" get pods -o wide
  kubectl -n "$_cluster_namespace" get svc
  local _rowsNumber="$( \
    kubectl get -n "$_cluster_namespace" pods -o wide \
    | awk '{print $7}' \
    | wc -l)"

  local _uniqueRowsNumber="$( \
    kubectl get -n "$_cluster_namespace" pods -o wide \
    | awk '{print $7}' \
    | sort \
    | uniq \
  ...
  if [ "$_rowsNumber" -gt "$_uniqueRowsNumber" ]; then
